...
MIGRATION_SQLALCHEMY_DATABASE_URL="sqlite:///./triangler_dev.db"
...
run-tests-native:
	zsh -c ". .venv/bin/activate && set -a && . ./test.env && set +a && cd src && pytest -vvv"

run-tests-native-loop:
	zsh -c ". .venv/bin/activate && set -a && . ./test.env && set +a && cd src && ptw . -vvv --now --patterns '*.py,pyproject.toml' --delay 0.5 --clear --color=yes"
...
generate-migrations:
	zsh -c "cd src && SQLALCHEMY_DATABASE_URL=${MIGRATION_SQLALCHEMY_DATABASE_URL} alembic revision --autogenerate"
